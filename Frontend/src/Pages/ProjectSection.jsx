...
import { GethData } from "../Redux/Slices/ProjectsSlice";

export function ProjectSection() {

    const {ProjectsData , isloading} = useSelector((state) => state.UserProjects);
      ...
      {ProjectsData.map((item, index) => (
        ...
        >
          ...
          <div className="flex flex-row  items-center  stat-title text-[12px]  ">
            ...
            <div className="ml-1 ">
            {item.projectLocation} , {item.projectCategory} , Posted · {timeAgo(item.postDate)}
            </div>
            ...
            <div className="block stat-title text-[16px] font-bold text-black">{item.projectDescription}</div>
          ...
          <div className="flex flex-row justify-around  h-16 items-center  ">
            ...
            <div className="  flex  flex-col text-center justify-center items-center  bg-gray-200 pr-6 pl-6 pt-1 pb-1 rounded-sm shadow w-42 ">
              ...
              <p className="stat-title text-gray-700 font-bold text-[12px]">Quotation Requests</p>
          {
            
            item.quotationRequests.map((q,index)=>(

             <div key={index} className=" bg-gray-150 w-full  h-14 shadow-sm rounded-sm  flex flex-row relative">
               <img className="  w-8 h-8 mt-2 flex justify-center items-center rounded-full ml-3 " src={q.companyLogo}  />
               ...
               <button className="absolute right-4 top-3 bg-gray-200 pl-3 pr-3 pt-1.5 pb-1.5 rounded-sm text-[12px] text-[#0c2b78] font-bold shadow-sm cursor-pointer " onClick={() => HandleURL(q.pdFurl)}>View PDF</button>
             </div>
            ))
          }
